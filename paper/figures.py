...
# figure.legend(handles, labels, ncols=4, loc='upper left')
...
setting = {'Single-head + LS': ('single', 'SLS'),
          'Multi-head + HLS': ('multi', 'HLS')}

arch = {'MycoAI-BERT': 'BERT-medium',
        'MycoAI-CNN': 'CNN-Vu'}
...
for i, x in enumerate(arch):
    ...
    for j, y in enumerate(setting):
        ...
        model_file = f'MycoAI-{setting[y][0]}-{arch[x]}-{setting[y][1]}.pt'
        model = torch.load(model_folder + model_file, utils.DEVICE)
        data = Data('/data/luuk/trainset_valid.fasta')
        unknown_data = Data('data/top50.fasta', allow_duplicates=True,
                            tax_parser=None)
        ...
        axs[i,j].scatter('Dim 1','Dim 2', s=3, label='Other', rasterized=True,
            data=data[(data['phylum']=='Other')|(data['phylum']=='?')])
        axs[i,j].scatter('Dim 1','Dim 2',data=unknown_data, rasterized=True,
                         color='black', s=1, label='Top 50 most wanted')
        axs[i,j].set_xticks([])
        axs[i,j].set_yticks([])
...
plt.savefig(f'cnn_vs_bert_latent_spaces.png', dpi=300)
...
#     'DNABarcoder': 'DNABarcoder',
#     'RDP classifier': 'RDP',
#     'MycoAI-CNN': 'MycoAI-multi-CNN-Vu-HLS',
#     'MycoAI-BERT': 'MycoAI-multi-BERT-medium-HLS'
# }
