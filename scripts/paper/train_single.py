...
valid_src = Data('/data/luuk/trainset_valid.fasta', allow_duplicates=True)
# train_src = Data('/data/luuk/trainset.fasta', allow_duplicates=True)
train = TensorData(filepath='/data/luuk/trainset_bpe.pt')
valid = TensorData(filepath='/data/luuk/trainset_bpe_valid.pt')

output = 'infer_sum'
lr = 0.0001
wd = 0

smoothing = [[0.020, 0.020, 0.020, 0.020, 0.020, 0.000]] 

for label_smoothing in smoothing:

    name = f'(Debug) MycoAI-single LS {" ".join([str(s) for s in label_smoothing])}'
    ...
    model = DeepITSTrainer.train(model, train, valid, 50, loss,
        levels=[0,0,0,0,0,1], optimizer=optim, batch_size=1,
